...
$(function() {
	...
	$.getJSON("/api/courses/" + courseId, function(data) {
        ...
        let regBtnTag = '<div class="text-center"><a class="btn btn-primary w-25 text-center" href="register.html?id=' + courseId + '" id="registerBtn">REGISTER FOR THIS COURSE</a></div>'
        $("#detailTblOutputForm").append(regBtnTag);

        if(data.Students.length > 0){
            $("#studentTblOutput").show();
            for(let i = 0; i < data.Students.length; i++){
            buildRow(data.Students[i].studentname, data.Students[i].email);
        }
        }
        else {
            $("#noStudentsDiv").show();
...
function buildRow(property, value) {
    ...
    let markup = 
    '<tr><td>' + property + '</td><td id="' + inputId + '">' + value + '</td></tr>'
